...
@section('plugins.Datatables', true)
    ...
    <div class="form-group col-md-6">
            ...
            <div class="form-group col-md-6"; align="right">
                <a class="btn btn-success" href="{{ route('add-teamhandover') }}"><i class="fas fa-plus-square"></i></a>
            </div>
            ...
            <div class="table-responsive">
                ...
                <table id="myTable" class="table table-bordered table-striped {{ count($type) > 0 ? 'datatable' : '' }} pointer">
                    ...
                    <tr>
                        ...
                        <th> Employee Name</th>
                        <th> Handover To Employee Name</th>
                        <th> Handover Reason</th>
                        <th> Handover Form Date</th>
                        <th> Handover To Date</th>
                    ...
                    @if (count($type) > 0)
                        @foreach ($type as $user)
                            <tr data-entry-id="{{ $user->id }}">
                                ...
                                <td>{{ $user->name }}</td>
                                <td>{{ $user->handover_reason }}</td>
                                <td>{{ $user->handover_from_date }}</td>
                                <td>{{ $user->handover_to_date }}</td>

                                
                             <td> <a href="{{ route('edit-teamhandover',[$user->id]) }}" class="btn btn-xs btn-info">
                                        ...
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    ...
                                                    <h4 class="modal-title">Team Handover </h4>
                                                </div>
                                                <div class="modal-body">
                                                    <p>Sure about delete this</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <a href="{{route('deleteteamhandover',['id'=>$user->id])}}" class="btn btn-xs btn-info"><i class="fa fa-trash fa-lg" aria-hidden="true"></i>
                                                        Delete</a>
                                                </div>
                                            </div>
                                        </div>
