...
@include('include.breadcrumbs', ['breadcrumbs' => [
    ...
    'Add Training' => route('add-employeetraining'),
...
<div class="card-body">
<form action="{{ url('submit-employeetraining') }}" method="POST">
@csrf
@if ($errors->any())
...
<td><input type="date" name="moreFields[0][training_date]" placeholder="Enter Training Date" class="form-control" /> 
<input type="hidden" name="moreFields[0][emp_id]" value="{{ Auth::user()->emp_id }}" class="form-control" /></td> 

<td><input type="time" name="moreFields[0][training_time]" placeholder="Enter Training Time" class="form-control" /></td>  
